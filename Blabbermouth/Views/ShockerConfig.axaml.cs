...
public partial class ShockerConfig : UserControl
{
    ...
    private async void TestCredentialsClicked(object? sender, RoutedEventArgs e)
    {
        ...
        if (result != null)
        {
            await MainWindow.ShowErrorAsync(result, "Test failed");
            return;
        }
    ...
    public async Task TestPortAsync()
    {
        ...
        if (ShockerID == 0)
        {
            await MainWindow.ShowErrorAsync("You must enter a shocker ID.", "Error");
            return;
        }
        ...
        {
            await MainWindow.ShowErrorAsync(
                "Access to the selected serial port is denied. " +
                "This can happen if another application is using the port, " +
